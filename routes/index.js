...
router.get('/loginu2fcar', function(req, res, next) {
    ...
    res.render('loginu2fcar.ejs', { message: req.flash('loginMessage') , user: req.user});
});
...
router.post('/loginbmw', passport.authenticate('local-bmw', {
    successRedirect: '/loginu2fcar',
    failureRedirect: '/loginbmw',
    failureFlash: true,
}));

router.post('/loginu2fcar', passport.authenticate('local-bmw', {
...
router.post('/authorize',  passport.authenticate('local-auth', {
    successRedirect: '/profile',
